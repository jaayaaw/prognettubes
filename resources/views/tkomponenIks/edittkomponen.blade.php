...
<div class="nk-fmg-body-head d-none d-lg-flex">
    ...
    <div class="nk-fmg-actions">
        <div class="btn-group">
            ...
            <a href="/tkomponeniks/{{ App\Models\KomponenIks::where('id',$id)->pluck('iks_provider_id')->first() }}/{{ $nama }}" class="btn btn-sm btn-primary" onclick="buttondisable(this)"><em class="icon fas fa-arrow-left"></em> <span>Kembali</span></a>
        </div>
    </div>
</div>
...
<div class="nk-fmg-quick-list nk-block">
    <div class="card">
        <div class="card-body">
        <h4>{{ $nama }}</h4><br>
        <form method="POST" action="{{ route('tkomponeniks.update', [$id, $nama]) }}" id="form1">
            @csrf
            @method('PUT')
            <div class="mb-3 row">
                <label for="nama" class="col-sm-2 col-form-label">IKS Provider ID</label>
                    <div class="col-sm-2">
                        <input type="text" class="form-control @error('iks_provider_id') is-invalid @enderror" id="nama" name="iks_provider_id" value="{{ $data['iks_provider_id'] }}" >
                    </div>
                ...
                <label for="nama" class="col-sm-2 col-form-label">Group Komponen</label>
                    <div class="col-sm-3">
                        <select class="form-control" name="iks_gkomponen_id" id="iks_gkomponen_id">
                            ...
                            <option value="{{$k->id}}" {{ $data->iks_gkomponen_id == $k->id ? 'selected' : '' }}>{{ $k->group }}</option>
                            @endforeach
                        </select>
                    </div>
