...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="/provideriks/{{ App\Models\ProviderIks::where('id',$id)->pluck('iks_id')->first() }}/{{ $nama }}" style="color: grey;">Provider IKS</a></li>
    <li class="breadcrumb-item active" aria-current="page" style="color: blue;">Komponen IKS</li>
  </ol>
</nav>
<br>
<div class="nk-fmg-body-head d-none d-lg-flex">
    ...
    <div class="nk-fmg-actions">
        <div class="btn-group">
            <a href="/provideriks/{{ App\Models\ProviderIks::where('id',$id)->pluck('iks_id')->first() }}/{{ $nama }}" class="btn btn-sm btn-secondary" onclick="buttondisable(this)"><em class="icon fas fa-arrow-left"></em> <span>Kembali</span></a>	&nbsp;
            <a href="{{ route('tkomponeniks.create', ['id' => $id, $nama]) }}" class="btn btn-sm btn-primary" onclick="buttondisable(this)"><em class="icon fas fa-plus"></em> <span>Add Data</span></a>
        </div>
    </div>
</div>
                ...
                <div class="table-responsive">
                    <table id="{{$table_id}}" class="small-table table " style="width:100%">
                        <thead style="color:#526484; font-size:11px;" class="thead-light">
                            ...
                            <th width="10%">IKS Group Komponen</th>
                            <th width="10%">Aksi</th>
                        </thead>
                        <tbody></tbody>
                    </table>
...
$(document).ready(function() {
    table = $('#{{$table_id}}').DataTable({
        ...
        ajax: {
            url: '{{ route("tkomponeniks.listData") }}',
            type:"POST",
        ...
        columns: [
            ...
            {
                data: 'provider_sel.nama_iks',
                name: 'iks_provider_id',
                orderable: true,
                searchable: true,
                class: 'text-left'
            },
            {
                data: 'gkomponen_sel.group',
