...
<div class="nk-fmg-body-head d-none d-lg-flex">
    ...
    <div class="nk-fmg-actions">
        <div class="btn-group">
            ...
            <a href="/iksgkdetail/{{ App\Models\MasterIksGroupKomponenDetail::where('id',$id)->pluck('gkomponen_id')->first() }}" class="btn btn-sm btn-primary" onclick="buttondisable(this)"><em class="icon fas fa-arrow-left"></em> <span>Kembali</span></a>
        </div>
    </div>
</div>
...
<div class="nk-fmg-quick-list nk-block">
    <div class="card">
        <div class="card-body">
        <form method="POST" action="{{ url('iksgkdetail') }}/{{ $data['id'] }}" id="form1">
            @csrf
            @method('PUT')
            <div class="mb-3 row">
                <label for="nama" class="col-sm-2 col-form-label">Group Komponen ID</label>
                    <div class="col-sm-3">
                    <select class="form-control" name="gkomponen_id" id="gkomponen_id" value="{{ $data->gkdetailSel->group }}">
                            <option value="" selected disabled>- Pilih Group Komponen -</option>
                            @foreach($iksgkomponen as $i)
                            <option value="{{  $i->id }}" {{ $data->gkomponen_id == $i->id ? 'selected' : '' }}> {{$i->group}}</option>
                            @endforeach
                    </select>
                    </div>
            </div>
